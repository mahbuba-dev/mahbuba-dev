<!-- Banner -->
<p align="center">
  <img src="/image/banner-mahbuba.png" alt="Mahbuba Akter Banner" />
</p>

<!-- Typing Animation -->
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?size=28&color=4E8D9C&center=true&vCenter=true&width=650&lines=Hi,+I'm+Mahbuba+Akter;Full‑Stack+Web+Developer;Clean+UI+%7C+Modern+UX+%7C+Scalable+Systems" />
</p>

---

# 👩‍💻 About Me
I’m a **Full‑Stack Web Developer** focused on building clean, scalable, and production‑ready applications.  
I love crafting beautiful UI, designing smooth user experiences, and building systems that feel premium.

- 💻 **Tech Focus:** React, Next.js, Node.js, PostgreSQL, MongoDB  
- 🎨 **Passion:** Clean UI, modern UX, minimal design  
- ⚡ **Goal:** Build SaaS platforms & real‑world digital products  
- 🌍 Live in **New York, USA**  
- 🗣️ Bangla & English  

---

# 🚀 Tech Stack 

<p align="center">
  <img src="/image/icons8-html-50" width="70" />
  <img src="/image/icons8-css-50.png" width="70" />
  <img src="/image/icons8-js-50.png" width="70" />
   <img src="/image/icons8-tailwindcss-50.png" width="70" title="shadcn/ui" />
  <img src="/image//icons8-react-50.png" width="70" />
  <img src="/image//icons8-nextjs-50.png" width="70" />
  <img src="/image/icons8-typescript-50.png" width="70" />
  <img src="/image/icons8-nodejs-50.png" width="70" />
  <img src="/image/icons8-express-js-50.png" width="70" />
  <img src="/image/icons8-postgresql-50.png" width="70" />
  <img src="/image/icons8-prisma-orm-50.png" width="70" title="shadcn/ui" />
  <img src="/image/icons8-mongodb-50.png" width="70" title="shadcn/ui" />
 
</p>

---

# 🧩 What I Do
- Build full‑stack applications with modern frameworks  
- Design clean, responsive, user‑friendly interfaces  
- Develop secure authentication systems  
- Architect scalable backend APIs  
- Create production‑ready SaaS platforms  
- Write clean, maintainable, modular code  

---

# ⭐ Featured Projects

### 🔹 **ConsultEdge — Expert Consultation SaaS**
**Tech:** Next.js, Node.js, Prisma, PostgreSQL, Stripe, WebSockets  
- Real‑time messaging  
- Role‑based dashboards  
- Stripe payments  
- Notification system  
- Secure authentication  

🔗 Live Demo | 🔗 Repo

---

### 🔹 **CareerCode — Full‑Stack Job Portal**
**Tech:** Next.js, Prisma, MongoDB, TanStack Query  
- Job posting & applications  
- User dashboards  
- Protected routes  
- Modern UI  

🔗 Live Demo | 🔗 Repo

---

### 🔹 **E‑Commerce CRUD App**
**Tech:** React, Express.js, MongoDB  
- Product CRUD  
- Filtering & pagination  
- Responsive UI  

🔗 Repo

---

# 🛠 Tools & Platforms (3D Icons)

<p align="center">
  >
  <img src="image/icons8-docker-50.png" width="70" />
  <img src="image/icons8-figma-50.png" width="70" />
  <img src="image/icons8-firebase-50.png" width="70" />
  <img src="image/icons8-vercel-50.png" width="70" />
</p>

---

# 📊 GitHub Stats

<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=Mahbuba-islam&show_icons=true&theme=transparent&title_color=4E8D9C&icon_color=85C79A" />
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Mahbuba-islam&layout=compact&theme=transparent&title_color=4E8D9C" />
</p>

---

# 🔥 Activity Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Mahbuba-islam&theme=github-compact&color=4E8D9C&line=85C79A&point=EDF7BD" />
</p>

---

# 🌐 Connect With Me

<p align="center">
  <a href="https://mahbuba-portfolio.vercel.app">
    <img src="https://img.shields.io/badge/Portfolio-4E8D9C?style=for-the-badge&logo=vercel&logoColor=white" />
  </a>
  <a href="https://github.com/Mahbuba-islam">
    <img src="https://img.shields.io/badge/GitHub-281C59?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/mahbuba-akter-020157211/">
    <img src="https://img.shields.io/badge/LinkedIn-85C79A?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:mahbubaislam7010@gmail.com">
    <img src="https://img.shields.io/badge/Email-EDF7BD?style=for-the-badge&logo=gmail&logoColor=black" />
  </a>
</p>

---

# 🔥 Keep Going Forward 🥰